  ...
  if (train) {
    ...
    if (isWhereQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_location", train: trainInfo }),
        response: g(base,
          `Train ${cardinal(train.number)} ${train.name} is at Platform ${cardinal(train.platform)}.`,
          `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} प्लेटफॉर्म ${cardinal(train.platform)} पर है।`,
          `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} ప్లాట్‌ఫారం ${cardinal(train.platform)}లో ఉంది.`),
      };
    }
    ...
    if (isPlatformQ && !isTimingQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_platform", train: trainInfo }),
        response: g(base,
          `${train.name}, train number ${cardinal(train.number)}, will arrive at Platform ${cardinal(train.platform)}.`,
          `${train.nameHindi}, ट्रेन नंबर ${cardinal(train.number)}, प्लेटफॉर्म ${cardinal(train.platform)} पर आएगी।`,
          `${train.nameTelugu}, రైలు నంబర్ ${cardinal(train.number)}, ప్లాట్‌ఫారం ${cardinal(train.platform)}కి వస్తుంది.`),
      };
    }
    ...
    if (isArrivalQ && !isDepartureQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_arrival", train: trainInfo }),
        response: g(base,
          `Train ${cardinal(train.number)} ${train.name} arrives at ${t(train.actualArrival)} on Platform ${cardinal(train.platform)}.`,
          `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} ${t(train.actualArrival)} प्लेटफॉर्म ${cardinal(train.platform)} पर आएगी।`,
          `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} ${t(train.actualArrival)} ప్లాట్‌ఫారం ${cardinal(train.platform)}కి వస్తుంది.`),
      };
    }
    ...
    if (isDepartureQ && !isArrivalQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_departure", train: trainInfo }),
        response: g(base,
         `Train ${cardinal(train.number)} ${train.name} departs at ${t(train.actualDeparture)} from Platform ${cardinal(train.platform)}.`,
         `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} ${t(train.actualDeparture)} प्लेटफॉर्म ${cardinal(train.platform)} से जाएगी।`,
         `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} ${t(train.actualDeparture)} ప్లాట్‌ఫారం ${cardinal(train.platform)} నుంచి వెళ్తుంది.`),
      };
    }
    ...
    if (isTimingQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_timing", train: trainInfo }),
        response: g(base,
          `Train ${cardinal(train.number)} ${train.name} arrives at ${t(train.actualArrival)} and departs at ${t(train.actualDeparture)} from Platform ${cardinal(train.platform)}.`,
          `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} ${t(train.actualArrival)} आएगी और ${t(train.actualDeparture)} प्लेटफॉर्म ${cardinal(train.platform)} से जाएगी।`,
          `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} ${t(train.actualArrival)} వస్తుంది మరియు ${t(train.actualDeparture)} ప్లాట్‌ఫారం ${cardinal(train.platform)} నుంచి వెళ్తుంది.`),
      };
    }
    ...
    if (/delay|late|देरी|लेट|ఆలస్యం/i.test(lower)) {
      return {
        ...
        response: g(base,
          train.delay > 0
            ? `Train ${cardinal(train.number)} ${train.name} is running ${cardinal(train.delay)} minutes late.`
            : `Train ${cardinal(train.number)} ${train.name} is running on time.`,
          train.delay > 0
            ? `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} ${cardinal(train.delay)} मिनट देरी से चल रही है।`
            : `ट्रेन नंबर ${cardinal(train.number)} ${train.nameHindi} समय पर चल रही है।`,
          train.delay > 0
            ? `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} ${cardinal(train.delay)} నిమిషాలు ఆలస్యంగా వస్తుంది.`
            : `ట్రెయిన్ నంబర్ ${cardinal(train.number)} ${train.nameTelugu} సమయానికి వస్తుంది.`),
      };
    }
    // ── Train number query ──────────────────────────────────────────────────────
    const isTrainNumberQ = /number|नंबर|నంబర్|no\.|संख्या|సంఖ్య/i.test(lower);
    if (isTrainNumberQ) {
      return {
        ...
        intentData: toDataString({ topic: "train_number", train: trainInfo }),
        response: g(base,
          `${train.name} train number is ${cardinal(train.number)}. It runs from ${train.from} to ${train.to} on Platform ${cardinal(train.platform)}.`,
          `${train.nameHindi} का ट्रेन नंबर ${cardinal(train.number)} है। यह ${train.from} से ${train.to} तक प्लेटफॉर्म ${cardinal(train.platform)} पर चलती है।`,
          `${train.nameTelugu} రైలు నంబర్ ${cardinal(train.number)}. ఇది ${train.from} నుండి ${train.to} వరకు ప్లాట్‌ఫారం ${cardinal(train.platform)}లో వస్తుంది.`),
      };
    }
    ...
    return {
      ...
      intentData: toDataString({ topic: "train_status", train: trainInfo }),
      response: g(base,
        `${train.name}, train number ${n(train.number)}, is at Platform ${cardinal(train.platform)}. Arrival: ${t(train.actualArrival)}, Departure: ${t(train.actualDeparture)}.`,
        `${train.nameHindi}, ट्रेन नंबर ${n(train.number)}, प्लेटफॉर्म ${cardinal(train.platform)} पर है। आगमन: ${t(train.actualArrival)}, प्रस्थान: ${t(train.actualDeparture)}।`,
        `${train.nameTelugu}, రైలు నంబర్ ${n(train.number)}, ప్లాట్‌ఫారం ${cardinal(train.platform)}లో ఉంది. రాక: ${t(train.actualArrival)}, నిష్క్రమణ: ${t(train.actualDeparture)}.`),
    };
  }
